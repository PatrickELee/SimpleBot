...
class Points(commands.Cog):
    ...
    async def start_bet_error(self, ctx, error):
        if isinstance(error, commands.CommandError):
            await ctx.channel.send('Some error in starting bet, make sure you are doing !start_bet <@streamer> <what is the bet?>.')
            ...
            if(amount <= 0):
                ...
                if(not discord_id in self.bets):
                    ...
                    if(point_count >= bet_amount + self.bets[discord_id][0]):
                        ...
                        await ctx.channel.send(f'Sucessfully increased bet of {at_user} to a total of {self.bets[discord_id]}')
                    else:
                        await ctx.channel.send(f'{at_user} Invalid bet: you bet more points than you have')
        else:
